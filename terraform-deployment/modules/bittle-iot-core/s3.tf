...
resource "random_uuid" "bc_landing_bucket_uuid" {
}
...
data "aws_iam_policy_document" "bc_landing_bucket_restrict_file_types" {
  statement {
    principals {
      type = "AWS"
      identifiers = [
    ...
    resources = [
      "${aws_s3_bucket.bc_landing_bucket.arn}/*.amr",
      "${aws_s3_bucket.bc_landing_bucket.arn}/*.flac",
      "${aws_s3_bucket.bc_landing_bucket.arn}/*.mp3",
      "${aws_s3_bucket.bc_landing_bucket.arn}/*.mp4",
      "${aws_s3_bucket.bc_landing_bucket.arn}/*.ogg",
      "${aws_s3_bucket.bc_landing_bucket.arn}/*.webm",
      "${aws_s3_bucket.bc_landing_bucket.arn}/*.wav",
    ]
  }
  statement {
    principals {
...
resource "aws_s3_bucket_policy" "bc_landing_bucket_restrict_file_types" {
  count  = var.bc_s3_enable_bucket_policy ? 1 : 0
  bucket = aws_s3_bucket.bc_landing_bucket.id
  policy = data.aws_iam_policy_document.bc_landing_bucket_restrict_file_types.json
}

# Input Bucket IAM Policy
data "aws_iam_policy_document" "bc_input_bucket_restrict_file_types" {
  statement {
    principals {
      type = "AWS"
      identifiers = [
    ...
    # Allows all S3 operations for files matching the below suffixes
    resources = [
      "${aws_s3_bucket.bc_input_bucket.arn}/*.amr",
      "${aws_s3_bucket.bc_input_bucket.arn}/*.flac",
      "${aws_s3_bucket.bc_input_bucket.arn}/*.mp3",
      "${aws_s3_bucket.bc_input_bucket.arn}/*.mp4",
      "${aws_s3_bucket.bc_input_bucket.arn}/*.ogg",
      "${aws_s3_bucket.bc_input_bucket.arn}/*.webm",
      "${aws_s3_bucket.bc_input_bucket.arn}/*.wav",
    ]
  }
}

# Input S3 Bucket Policy
resource "aws_s3_bucket_policy" "bc_input_bucket_restrict_file_types" {
  count  = var.bc_s3_enable_bucket_policy ? 1 : 0
  bucket = aws_s3_bucket.bc_input_bucket.id
  policy = data.aws_iam_policy_document.bc_input_bucket_restrict_file_types.json
}
...
data "aws_iam_policy_document" "bc_app_storage_bucket_restrict_file_types" {
  statement {
    principals {
      type        = "AWS"
      identifiers = ["${data.aws_caller_identity.current.account_id}"]
    ...
    # Allows all S3 operations for files matching the below suffixes
    resources = [
      "${aws_s3_bucket.bc_app_storage_bucket.arn}/*.json",
      "${aws_s3_bucket.bc_app_storage_bucket.arn}/*.amr",
      "${aws_s3_bucket.bc_app_storage_bucket.arn}/*.flac",
      "${aws_s3_bucket.bc_app_storage_bucket.arn}/*.mp3",
      "${aws_s3_bucket.bc_app_storage_bucket.arn}/*.mp4",
      "${aws_s3_bucket.bc_app_storage_bucket.arn}/*.ogg",
      "${aws_s3_bucket.bc_app_storage_bucket.arn}/*.webm",
      "${aws_s3_bucket.bc_app_storage_bucket.arn}/*.wav",
    ]
  }
}
# # - App Storage S3 Bucket Policy -
resource "aws_s3_bucket_policy" "bc_app_storage_bucket_restrict_file_types" {
  count  = var.bc_s3_enable_bucket_policy ? 1 : 0
  bucket = aws_s3_bucket.bc_app_storage_bucket.id
  policy = data.aws_iam_policy_document.bc_app_storage_bucket_restrict_file_types.json
}

# S3 Bucket Notifications -> EventBridge
resource "aws_s3_bucket_notification" "bc_landing_bucket_events" {
  bucket = aws_s3_bucket.bc_landing_bucket.id
  # Send all S3 events for this bucket to EventBridge
  eventbridge = true
}
# S3 Bucket Notifications -> EventBridge
resource "aws_s3_bucket_notification" "bc_input_bucket_events" {
  bucket = aws_s3_bucket.bc_input_bucket.id
  # Send all S3 events for this bucket to EventBridge
  eventbridge = true
}
...
resource "aws_s3_bucket_notification" "bc_app_storage_bucket_events" {
  bucket = aws_s3_bucket.bc_app_storage_bucket.id
  # Send all S3 events for this bucket to EventBridge
  eventbridge = true
}
...
resource "aws_s3_bucket_lifecycle_configuration" "bc_landing_bucket_lifecycle_config" {
  ...
  rule {
    ...
    expiration {
      days = var.bc_landing_bucket_days_until_objects_expiration
    }
    status = "Enabled"
  }
}
resource "aws_s3_bucket_lifecycle_configuration" "bc_input_bucket_lifecycle_config" {
  count  = var.bc_input_bucket_create_nuke_everything_lifecycle_config ? 1 : 0
  bucket = aws_s3_bucket.bc_input_bucket.id
  rule {
    id = "nuke-everything"
    filter {} // empty filter = rule applies to all objects in the bucket
    expiration {
      days = var.bc_input_bucket_days_until_objects_expiration
    }
    status = "Enabled"
  }
}
...
# - S3 Buckets -
# S3 Landing Bucket
resource "aws_s3_bucket" "bc_landing_bucket" {
  bucket        = "${var.bc_landing_bucket_name}-${random_uuid.bc_landing_bucket_uuid.result}"
...
resource "aws_s3_bucket_public_access_block" "bc_landing_bucket_block_public_access" {
...
resource "aws_s3_bucket_cors_configuration" "bc_landing_bucket_cors" {
  count  = var.bc_landing_bucket_enable_cors ? 1 : 0
  bucket = aws_s3_bucket.bc_landing_bucket.id
...
# S3 Input Bucket
resource "aws_s3_bucket" "bc_input_bucket" {
  bucket        = "${var.bc_input_bucket_name}-${random_uuid.bc_input_bucket_uuid.result}"
  force_destroy = var.s3_enable_force_destroy
...
resource "aws_s3_bucket_public_access_block" "bc_input_bucket_block_public_access" {
  count               = var.bc_s3_block_public_access ? 1 : 0
  bucket              = aws_s3_bucket.bc_input_bucket.id
  block_public_acls   = var.bc_s3_block_public_acls
  block_public_policy = var.bc_s3_block_public_policy
...
resource "aws_s3_bucket_cors_configuration" "bc_input_bucket_cors" {
  count  = var.bc_input_bucket_enable_cors ? 1 : 0
  bucket = aws_s3_bucket.bc_input_bucket.id
...
resource "aws_s3_bucket" "bc_app_storage_bucket" {
  bucket        = "${var.bc_app_storage_bucket_name}-${random_uuid.bc_app_storage_bucket_uuid.result}"
  force_destroy = var.s3_enable_force_destroy
...
resource "aws_s3_bucket_public_access_block" "bc_app_storage_bucket_block_public_access" {
  bucket              = aws_s3_bucket.bc_app_storage_bucket.id
  block_public_acls   = var.bc_s3_block_public_acls
  block_public_policy = var.bc_s3_block_public_policy
}
# S3 Amplify App Storage Bucket - CORS Policy
resource "aws_s3_bucket_cors_configuration" "bc_app_storage_bucket_cors" {
  bucket = aws_s3_bucket.bc_app_storage_bucket.id
